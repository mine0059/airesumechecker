...
const responseSchema = {
  ...
  properties: {
    ...
    summary: { type: Type.STRING, description: "One short paragraph overall verdict" },
  },
};
...
const analysisValidator = z.object({
  ...
  .min(1),
  bulletRewrite: z.array(z.object({ section: z.string(), original: z.string(), rewritten: z.string(), rationale: z.string() }))
  .default([]),
...
function buildPrompt({ rawText, targetRole }) {
  return [
    ...
    targetRole
     ...
     "Rewrite exactly 5 prioritized issues, 5 standout strengths, and 5-10 weak bullets rewritten to be stronger, quantified, and ATS-friendly.",
    "Rewrite must preserve the original meaning. Each rewrite needs a one-line rationale.",
