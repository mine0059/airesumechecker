...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const userId = req.user._id; // fix: req.user_id → req.user._id

    const resumes = await Resume.find({ userId })
      .sort({ updatedAt: -1 })
      .lean();
    const resumeMap = new Map(resumes.map((r) => [r._id.toString(), r])); // fix: resume → resumes
    ...
    const scoreTrend = analyses.map((a) => ({
      ...
      score: a.atsScore,
      resumeId: a.resumeId,
      // fix: resumeMap.get(...?.title) → resumeMap.get(...)?.title
