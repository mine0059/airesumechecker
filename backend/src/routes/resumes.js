  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const [resume] = await Resume.create(
        [{ userId: req.user._id, title, latestVersionNumber: 1 }],
        { session },
      );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const saved = await Analysis.create({
      ...
      bulletRewrites: analysis.bulletRewrite,
...
router.get(
  ...
  validate(z.object({ id: objectIdSchema, versionId: objectIdSchema }), 'params'),
  asyncHandler(async (req, res) => {
    const resume = await loadOwnedResume(req);
    const version = await loadVersion(resume._id, req.params.versionId);
    const analysis = await Analysis.findOne({ resumeId: resume._id, versionId: version._id })
      .sort({ createdAt: -1 })
      .lean();
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const selected = req.body.rewriteIds?.length
      ? analysis.bulletRewrites.filter((r) => req.body.rewriteIds.includes(r._id.toString()))
    ...
    // Safety net: pre-build a structured copy from the base   version with
    // the chosen bullets swapped in, so v2  never lands with empty sections even if
